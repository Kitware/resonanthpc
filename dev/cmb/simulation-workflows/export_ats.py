...
def import_python_op(op_manager, path):
    ...
    if import_outcome != OPERATION_SUCCEEDED:
        raise RuntimeError(
            "Failed to load export operator: {}, outcome {}".format(
                path, import_outcome
            )
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate ATS input file from attribute resource"
    )
    parser.add_argument(
        "attribute_filepath", help="Input attribute filename/path (.smtk)"
    )
    parser.add_argument(
        "--model_filepath", "-m", help="path to SMTK model resource (.smtk)"
    )
    parser.add_argument(
        "--output_filepath",
        "-o",
        default="ats_input.xml",
        help="output filename/path (ats_input.xml)",
    )
    parser.add_argument(
        "--export_operation",
        "-e",
        default="ats/ats.py",
        help="export operation path (./ats/ats.py)",
    )
    ...
    if args.model_filepath:
        ...
        assert (
            model_resource is not None
        ), "failed to load model resource from file {}".format(mfile)
